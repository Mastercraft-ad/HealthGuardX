...
export class DatabaseStorage implements IStorage {
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    // Some database drivers (like the SQLite dev fallback) do not support
    // Postgres-specific default expressions such as gen_random_uuid(). To
    // keep inserts portable, ensure an `id` is present before inserting.
    if (!insertUser.id) {
      (insertUser as any).id = randomUUID();
    }
    ...
    try {
      ...
      console.error('createUser insert returning failed, falling back. Error:', err && err.stack ? err.stack : err);

      // Fall back to inserting without returning and then selecting the created row
      await db.insert(users).values(insertUser);

      // Try to fetch by walletAddress first
      if (insertUser.walletAddress) {
        const found = await this.getUserByWalletAddress(insertUser.walletAddress as string);
        if (found) return found;
      }

      // Then try by uid
      if (insertUser.uid) {
        const found = await this.getUserByUid(insertUser.uid as string);
        if (found) return found;
      }

      // As a last resort throw a clearer error
      throw new Error('Failed to create user (insert returned error and fallback lookup failed)');
    }
  }
  ...
  async createAuditLog(log: InsertAuditLog): Promise<AuditLog> {
    // Ensure an id is present (some DBs don't apply server-side defaults)
    if (!log.id) {
      (log as any).id = randomUUID();
    }
    ...
    try {
      ...
      console.error('createAuditLog returning failed, falling back, error:', err && err.stack ? err.stack : err);
      await db.insert(auditLogs).values(log);
      const [found] = await db.select().from(auditLogs).where(eq(auditLogs.id, log.id as string));
      if (found) return found as AuditLog;
